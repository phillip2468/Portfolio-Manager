  ...
  return useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        cell: row => {
          const changePercentage = (100 - ((row.units_price / row.stock_details.market_current_price) * 100)).toFixed(2)
          return (
            <div style={{ color: changePercentage > 0 ? 'green' : 'red' }}>{changePercentage}</div>
          )
        }
      }
    ]
