...
@predictor_bp.route("/past-data")
def past_data():
    prediction_of_prices = []
    dictionary_stuff = []
    stock = "CBA.AX"
    training_data_prices = Ticker(stock).history(interval="1d", start="2015-01-01", end="2020-6-30")
    test_data_prices = Ticker(stock).history(interval="1d", start="2021-01-01", end="2021-12-30")
    ...
    with tf.device('/cpu:0'):
        ...
        for i in range(1, 2):
            ...
            prediction_of_prices.append([str(element) for element in prediction.flat])
            dictionary_stuff.append(dictionary)
            print(prediction_of_prices)
    print(prediction_of_prices)
    return jsonify(dictionary_stuff[-1])
