...
@pytest.mark.repeat(REPEAT_TESTS)
def test_invalid_add_empty_portfolio_name(flask_application: FlaskClient, user_account_logged_in: dict, user_id: int) \
        -> None:
    """
    GIVEN a portfolio name that is empty
    WHEN a user is logged in and wants to create a portfolio
    THEN check that the new portfolio is not created.
