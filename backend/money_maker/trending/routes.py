...
def ticker_information(exchange, market_cap, order):
    stmt: select = select(tP.symbol, tP.market_change_percentage, tP.market_cap, cast(func.timezone('AESST', cast(tP.last_updated, sqlalchemy.TIME)), sqlalchemy.Text))\
        .where(tP.market_change_percentage.isnot(None))\
        .where(tP.market_cap > market_cap)\
        .where(tP.exchange == exchange)\
        .order_by(tP.market_change_percentage.asc())\
        .limit(10)
    print(stmt)
